...
// Raised to 1_172_150 for the refusal-footer change: settle.ts gains the
// decorateFooter extension point and refuse's ownership of the model-visible
// cap, plus the CONTEXT.md Settle entry naming both decorators and the
// changelog entries for the two worktree refusals that lost their recovery
// pointer. worktree.ts shrinks (eight hand-written footers deleted), so this
// is interface and prose, not new machinery. Same verification: the file set
// is unchanged and none from tests/, scripts/, or evals/ (the new coverage is
// tests/settle-footer.test.ts, which is not packaged). Headroom is ~469 B over
// the measured 1_171_681, matching the previous raises.
assert.ok(pack.unpackedSize < 1_172_150, `package unpacked size too large: ${pack.unpackedSize}`);
