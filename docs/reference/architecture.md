...
_Modules_: `flow.ts`, `run.ts`, `settle.ts`, `delegation.ts`, `handoff.ts`, `handoff-types.ts`, `handoff-consumption.ts`, `return-types.ts`, `approval.ts`, `agent-profile.ts`, `budget.ts`, `integration.ts`, `contract-resolution.ts`, `validate.ts`, `validate-workflow.ts`, `bash-readonly.ts`, `sanitize.ts`, `trace.ts`, `trace-scope.ts`, `trace-sink.ts`, `trace-verify.ts`, `trace-attributes.ts`, `trace-structure.ts`, `trace-report.ts`, `trace-identity.mjs`.
...
Adding a module means classifying it here. Changing a Core module means either re-running the `/domain-driven-design` review and re-recording `docs/domain-review.json`, or accepting that the judgment rows report as stale on the PR — the same rule the codebase applies to an approval receipt whose digest no longer matches, and to trace health versus execution success: a carried fact is not repeated as current fact once the ground it was measured against has moved.
