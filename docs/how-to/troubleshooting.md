...
spend is that observation. The error names the ceiling that cannot pay, and no
worker was spawned against it. This is a projection, not spent budget: a budget
...
an unaffordable plan gets a bounded replan before it is refused. Keep the
configured ceiling unless the user explicitly approves a change.
