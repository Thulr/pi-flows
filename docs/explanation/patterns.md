...
- **Structured-output decoding for control tokens.** Verdict/loop/route/subtask/score decisions are parsed from the child's free text with anchored, fail-safe parsers: a control token is authoritative only on the child's first non-empty line, matching the exact documented grammar, and every unparseable form falls back safely (verdict → REVISE, loop → CONTINUE, route → unresolved, score → unscored). Constrained decoding (`structured-generation`) would make them airtight, but that needs host-side support in `pi`; the fail-safe fallbacks are the mitigation until then.
