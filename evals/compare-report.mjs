...
const exclusionText = (arm) => {
	...
	if (arm.exclusion.reason === "debug_budget") return `debug budget ${formatDuration(arm.timeoutPlan.effectiveTimeoutMs)} < case budget ${formatDuration(arm.timeoutPlan.caseTimeoutMs)}`;
...
export function armLine(label, arm) {
	...
	return `   ${label}  judge ${judgeScore}${arm.judged?.verdict === false ? "!" : ""}  obj ${objScore}${!arm.exclusion && arm.objective.pass ? "" : arm.exclusion ? "" : "!"}  $${arm.cost.toFixed(4)}  ${(arm.durationMs / 1000).toFixed(1)}s${excluded}`;
}
