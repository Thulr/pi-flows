...
import { appendReturnRequirements, canMutateWorkspace, clampIterations, clampLoopIterations, currentFlowDepth, validateConcurrency, validateSharedWriteCwd, writeCapabilityReason } from "./validate.ts";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: [
			...
			`Raw shapes: one delegated scout => {"agent":"recon","task":"inspect X","why":"..."}; parallel fan-out => {"tasks":[{"agent":"recon","task":"inspect A","tier":"fast"},{"agent":"analyst","task":"judge B","tier":"deep"}],"why":"..."}; build with separate critique => {"task":"...","evaluate":{},"why":"..."}. Further modes (${RUN_MODE_NAMES.filter((mode) => !["single", "parallel", "evaluate"].includes(mode)).join(", ")}) are documented in their parameter schemas.`,
		...
		promptGuidelines: [
			...
			"A raw parallel call with two or more tasks is refused before any child spends tokens unless every task names a tier/model, or the call names one flow-wide tier/model as an explicit choice to size every task uniformly. Prefer per-task tiers for mixed work; use a flow-wide tier only when the tasks intentionally need the same capability. Thinking alone is not model sizing.",
			...
			"Use debate only when the user explicitly requests opposing advocates, rebuttal, or adjudication; direct execution has matched its quality with lower cost. Use worktree only for multiple write-capable agents needing a gate-checked integration branch; use ordinary parallel for read-only fan-out. Use monitor only for bounded polling inside one flow call, never as durable scheduling.",
		...
		async execute(toolCallId, params, signal, onUpdate, ctx) {
			...
			const admission = await Flow.admit({
				...
				// The mode table's critical-path resolver: Core trace summaries
