...
- Live TUI monitoring: the `flow` tool row updates in place while children run (per-run state, current activity, progress, cost rollup); `F8` toggles a non-modal fleet panel showing every live run with budget burn-down; `/flows inspect` drills into one child's activity feed. Fan-out headers count **settled** runs (`1/3 settled` — see the [glossary](./CONTEXT.md#delegation-model)) and switch to the verdict (`2 failed`, `3 ok`) once nothing is outstanding. A durable flow card stays in the transcript after each flow, surviving session reloads. None of these interrupt children when closed.
...
automatically replay the same Flow without asking for direction or making a
material, visible change to the owning budget, Task, or fan-out.
