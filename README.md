...
Resumable workflow consent is bound to each gated Role's effective Agent profile: selected source, non-disclosing prompt identity, effective tools, resolved cwd, concrete model, and Thinking level. Profile drift reopens unspent approval rather than running under old consent; malformed or replayed receipts stay hard failures. See [Approval receipts](./docs/reference/flow-reference.md#approval-receipts).
