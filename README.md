...
Delegation is bounded on count, concurrency, time, nesting depth, and spend. The disclosed ceilings are `maxCostUsd` (cost), `maxTokens` (input plus output), and `maxGeneratedTokens` (output only — not input, context, total tokens, or cost). Cost, generated tokens, and a contract's total tokens stop live runs. A flow's `maxTokens` is a spawn gate only. At 80% of a live-run ceiling, live children are steered to wrap up and emit a partial return envelope. If the flow crosses a ceiling before the steer reaches the child, the run stops hard (`BUDGET_EXCEEDED`). Orchestrate also projects an admitted Decomposition against the remaining ceilings before workers spawn — each subtask can carry a relative `effortWeight` — and refuses an unaffordable plan (`BUDGET_HEADROOM_EXCEEDED`) instead of spending workers into a ceiling. Concurrent write-capable agents must not share one `cwd` (`SHARED_WRITE_CWD`) unless you explicitly allow it.
