...
Delegation is bounded on count, concurrency, time, and nesting depth, and on spend: `maxCostUsd` caps reported cost, `maxTokens` caps cumulative input+output, and `maxGeneratedTokens` caps generated output only (not input, model context, total tokens, or cost), with every configured ceiling disclosed before work starts. At 80% of a ceiling that stops live runs (cost or generated tokens, plus a contract's total tokens - a flow's `maxTokens` is a spawn gate only), live children are steered to wrap up and emit a partial return envelope; a ceiling crossed before the steer could reach the child is a hard stop (`BUDGET_EXCEEDED`). Concurrent write-capable agents may not share one `cwd` (`SHARED_WRITE_CWD`) unless explicitly allowed.
