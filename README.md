...
Delegation is bounded on count, concurrency, time, and nesting depth, and on spend: `maxCostUsd` / `maxTokens` / `maxGeneratedTokens` cap the whole flow, with every configured ceiling disclosed before work starts. At 80% of a ceiling the live child is steered to wrap up and emit a partial return envelope; a ceiling crossed with no chance to wrap up is a hard stop (`BUDGET_EXCEEDED`). Concurrent write-capable agents may not share one `cwd` (`SHARED_WRITE_CWD`) unless explicitly allowed.
