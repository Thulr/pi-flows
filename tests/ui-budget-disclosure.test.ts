...
test("budget disclosure finds flow and nested contract ceilings without inventing defaults", () => {
	...
	assert.equal(formatBudgetCeiling(ceilings[0]!), "flow ceiling: $0.25 cost · 9.0k total input+output tok");
	assert.equal(formatBudgetCeiling(ceilings[1]!), "contract ceiling: 2.0k generated-output-only tok [not total/input/context/cost]");
...
test("the call and live compact views disclose generated ceilings before usage exists", () => {
	...
	assert.match(call, /budget · contract ceiling: 2\.0k generated-output-only tok \[not total\/input\/context\/cost\]/);
	...
	assert.match(live, /budget · contract ceiling: 2\.0k generated-output-only tok \[not total\/input\/context\/cost\]/);
});

test("a preset call previews the ceilings generated by expansion", () => {
	const rendered = flowTool().renderCall(
		{ preset: "code-review", task: "Review HEAD against main.", why: "independent verification" },
		theme,
		{ cwd: process.cwd() },
	);
	const text = rendered.text ?? String(rendered);
	assert.match(text, /flow preset code-review \[user\]/);
	assert.match(text, /budget · flow ceiling: 750k total input\+output tok · 30k generated-output-only tok/);
	assert.match(text, /budget · contract ceiling: 300k total input\+output tok · 12k generated-output-only tok/);
});
...
test("exhausted live and durable views retain the binding authority and ceiling", () => {
	...
	assert.match(live, /BUDGET_EXCEEDED · contract ceiling: 2\.0k generated-output-only tok/);
	...
	assert.match(card, /BUDGET_EXCEEDED · contract ceiling: 2\.0k generated-output-only tok/);
});
