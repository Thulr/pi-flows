...
import { traceSpansForCase, gateBlocks, gateArgs, compareArgs, duelArgs, judgeArgs, calibrateArgs, inspectTraceArgs, labelFailuresArgs, formatGateScoreSummary, gateCandidateForEvalRun, evalRunDimensions, sharedGateDimensions } from "../evals/thulr.mjs";
...
// `--format junit` (thulr 0.3.0) replaces the terminal report on stdout with a
...
// Judge repeat-sampling (thulr 0.3.0): `--samples N` judges each case N times and
...
test("trace inspection, failure labels, and calibration args match thulr 0.3.0", () => {
