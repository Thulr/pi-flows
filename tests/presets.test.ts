...
test("project presets use the project-agent trust gate in headless calls", async () => {
	...
	assert.equal(result.details.trace, undefined, "a caller that asked for no trace gets none");

	// The caller's own trace still records the refusal, like every other pre-spawn refusal.
	const callerTraceFile = path.join(repo, "caller-trace.jsonl");
	const traced = await tools.get("flow").execute(
		"preset-trust-traced",
		{ preset: "local-review", task: "secret=must-not-leak", why: "test", agentScope: "project", traceFile: callerTraceFile },
		new AbortController().signal,
		undefined,
		{ cwd: repo, hasUI: false, ui: { confirm: async () => false, notify() {} } },
	);
	assert.equal(traced.details.error.code, "PROJECT_PRESET_APPROVAL_REQUIRED");
	assert.ok(traced.details.trace?.traceId, "the refusal is correlatable");
	const spans = (await readFile(callerTraceFile, "utf8")).split("\n").filter(Boolean).map((line) => JSON.parse(line));
	const root = spans.find((span) => span.parent_span_id === null);
	assert.equal(root.attributes["flow.refused_before_spawn"], "PROJECT_PRESET_APPROVAL_REQUIRED");
	assert.ok(spans.some((span) => span.attributes?.["flow.approval.decision"] === "required" && span.attributes?.["flow.preset"] === "local-review"));
	await assert.rejects(readFile(traceFile), "the preset's own trace file stays untrusted");
});
