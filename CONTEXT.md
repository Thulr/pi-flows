...
A toolset token granting a child bash restricted to a read-only command allowlist — git and file inspection plus repo verification — enforced by the pi-flows extension inside the child rather than requested by prompt. Classified not write-capable for the shared-write guard, and refused (`BASH_READONLY_UNENFORCEABLE`) when the child cannot load the enforcer. Coordination safety against ad-hoc mutations of a shared checkout, not a sandbox.
